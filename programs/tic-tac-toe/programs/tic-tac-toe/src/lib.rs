...
mod tests {
    ...
    #[test]
    fn game_serde_len() {
        assert_eq!(1, mem::size_of::<A>());
        assert_eq!(1, mem::size_of::<Option<A>>());
        assert_eq!(1, mem::size_of::<u8>());
        assert_eq!(2, mem::size_of::<Option<u8>>());

        let mut game = Game::default();
        game.turn = 1;
        assert_eq!(75, game.try_to_vec().unwrap().len());
        game.board = [
        ...
        game.turn = 2;
        assert_eq!(76, game.try_to_vec().unwrap().len());
        game.board = [
            [Some(Sign::X), Some(Sign::X), Some(Sign::X)],
            [Some(Sign::O), Some(Sign::O), Some(Sign::X)],
            [Some(Sign::O), Some(Sign::X), Some(Sign::O)],
        ];
        game.state = GameState::Won {
            winner: Pubkey::new_unique(),
        };
        game.turn = 10;
        assert_eq!(mem::size_of::<Game>() + 9, game.try_to_vec().unwrap().len());
        assert_eq!(107, mem::size_of::<Game>());
    }
}
